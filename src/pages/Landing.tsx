    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* Desktop contact boxes: side by side */
        .contact-boxes {
          display: flex;
          gap: var(--gap);
          flex: 0 0 auto;
        ...
        /* ════════════════════════════════════════════════════════════
           MOBILE PORTRAIT — strictly phone-sized, not tablet
           Uses max-width + orientation to be very specific
           ════════════════════════════════════════════════════════════ */
        @media (max-width: 480px) and (orientation: portrait),
          ...
          :root {
            ...
            --pad:     clamp(4px, 1.2vw, 7px);
            --radius:  clamp(8px, 1.8vw, 12px);
            --fs-ui:   clamp(8px, 2.2vw, 11px);
            --pad-box: clamp(7px, 1.8vw, 12px);
          }

          /* 
            Two-column grid: left ~34%, right fills rest.
            Both columns fill exact viewport height between header + footer.
          */
          .bento-area {
            display: grid;
          ...
          /* ── LEFT COLUMN — same structure as desktop ── */
          .left-col {
            display: flex;
          ...
          /*
            About photo: eats ~60% of left column height.
            Contact boxes share the remaining ~40%.
            These feel right proportionally — photo is the focal point.
          */
          .about-box {
            flex: 5 5 0;
            min-height: 0;
          ...
          /* ── RIGHT COLUMN — vertical stack of all boxes ── */
          .right-grid {
            display: flex;
            flex-direction: column;
            gap: var(--gap);
          ...
          /* Desktop rows become hidden — we use mobile-only wrappers */
          .top-row,
          .bottom-row {
            display: contents; /* children escape into right-grid flow */
          }

          /*
            Height distribution across 6 stacked items:
            Desktop inspiration:
              - Education  ~small  (top-left of top-row, 30% width, 50% height)
              - Experience ~large  (top-right of top-row, 70% width, 50% height)
              - Projects   ~large  (bottom-left, 70% width, 50% height)
              - LinksPanel ~medium (bottom-right, 30% width, 50% height)

            LinksPanel itself has 3 sub-rows: social icons (30%), commit (40%), resume+map (30%)
            We surface these as individual flex items using the mobile-links-stack class.

            Final vertical proportion (flex values out of ~18 total):
              Education   : 2   (~11%) — compact label box
              Experience  : 3   (~17%) — slightly taller
              Projects    : 3   (~17%) — same weight as experience
              Social row  : 2   (~11%) — icon strip
              Commit strip: 3   (~17%) — needs room for 3 lines of text
              CV + Map    : 3   (~17%) — two side-by-side small boxes

            Note: LinksPanel renders these internally — we size its container
            and let LinksPanel's own CSS handle the sub-row proportions.
          */

          /* Education — compact, label only */
          .mobile-edu  { flex: 3 3 0; min-height: 0; }
          /* Experience — slightly taller */
          .mobile-exp  { flex: 4 4 0; min-height: 0; }
          /* Projects — same weight */
          .mobile-proj { flex: 4 4 0; min-height: 0; }
          /* LinksPanel — trimmed down, still needs room for 3 sub-rows */
          .mobile-links { flex: 6 6 0; min-height: 0; }

          /* Ensure ClickableBox fills its flex slot */
          .mobile-edu  > .box,
          .mobile-exp  > .box,
          .mobile-proj > .box {
            height: 100%;
          ...
          /* ClickableBox renders as the root div.box itself — target directly */
          .right-grid > .mobile-edu,
          .right-grid > .mobile-exp,
          .right-grid > .mobile-proj {
            /* already handled by flex: above */
          }
        }
      `}</style>
      ...
      <div className={`${theme} page`}>
        ...
        <div className="bento-area">
              ...
              {[
                ...
                return (
                  <div
                    ...
                    onClick={handleCopy}
                    style={{ cursor: "copy", justifyContent: "center", gap: "clamp(4px,0.4vw,7px)" }}
          ...
          <div className="right-grid">
            {/*
              DESKTOP: top-row and bottom-row are grid containers.
              MOBILE:  top-row/bottom-row use `display: contents` so their
                       children bubble up into right-grid's flex flow,
                       where we can size them via .mobile-* wrapper divs.

              We wrap each child in a sizing div that's only relevant on mobile
              (on desktop, .top-row / .bottom-row grid overrides all sizing).
            */}
            <div className="top-row">
              <div className="mobile-edu">
                <ClickableBox
                  label="Education"
                  onClick={() => setIsEducationOpen(true)}
                  animationDelay="0.10s"
                  className="anim-education"
                  style={{ height: "100%" }}
                />
              </div>
